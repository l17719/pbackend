...
namespace System.Linq.Dynamic
{
    ...
    internal class Signature : IEquatable<Signature>
    {
        ...
        public Signature(IEnumerable<DynamicProperty> vproperties)
        {
            //if (vproperties != null) Properties = vproperties.ToArray();
            //hashCode = 0;
            //if (vproperties != null)
            //    foreach (var p in vproperties)
            //    {
            //        hashCode ^= p.Name.GetHashCode() ^ p.VType.GetHashCode();
            //    }

            if (vproperties != null)
            {
                Properties = vproperties.ToArray();
            }
            hashCode = 0;
            if (vproperties != null)
                foreach (var p in vproperties)
        ...
        public bool Equals(Signature other)
        {
            ...
            //}
            return !Properties.Where((t, i) => t.Name != other.Properties[i].Name || t.VType != other.Properties[i].VType).Any();
        }
    }
    ...
    internal class ClassFactory
    {
        ...
        FieldInfo[] GenerateProperties(TypeBuilder tb, DynamicProperty[] properties)
        {
            ...
            {
                var dp = properties[i];
                var fb = tb.DefineField("_" + dp.Name, dp.VType, FieldAttributes.Private);
                var pb = tb.DefineProperty(dp.Name, PropertyAttributes.HasDefault, dp.VType, null);
                var mbGet = tb.DefineMethod("get_" + dp.Name,
                    MethodAttributes.Public | MethodAttributes.SpecialName | MethodAttributes.HideBySig,
                    dp.VType, Type.EmptyTypes);
...
#pragma warning restore 0219
        ...
        Expression ParseIntegerLiteral()
        {
            ...
            var valueText = _token.Text;
            if (valueText[0] != '-')
            {
                ulong value;
                if (!UInt64.TryParse(valueText, out value))
                    throw ParseError(Res.InvalidIntegerLiteral, valueText);
                NextToken();
                if (value <= (ulong)Int32.MaxValue) return CreateLiteral((int)value, valueText);
                if (value <= (ulong)UInt32.MaxValue) return CreateLiteral((uint)value, valueText);
                if (value <= (ulong)Int64.MaxValue) return CreateLiteral((long)value, valueText);
                return CreateLiteral(value, valueText);
            }
            else
            {
                long value;
                ...
                if (value >= Int32.MinValue && value <= Int32.MaxValue)
                    return CreateLiteral((int)value, valueText);
                return CreateLiteral(value, valueText);
            }
        }
        ...
        Expression ParseRealLiteral()
        {
            ...
            var valueText = _token.Text;
            object value = null;
            char last = valueText[valueText.Length - 1];
            if (last == 'F' || last == 'f')
            {
                float f;
                if (Single.TryParse(valueText.Substring(0, valueText.Length - 1), out f)) value = f;
            }
            else
            {
                double d;
                if (Double.TryParse(valueText, out d)) value = d;
            }
            if (value == null) throw ParseError(Res.InvalidRealLiteral, valueText);
            NextToken();
            return CreateLiteral(value, valueText);
        }

        Expression CreateLiteral(object value, string valueText)
        {
            ConstantExpression expr = Expression.Constant(value);
            literals.Add(expr, valueText);
            return expr;
        }
        ...
        static Type FindGenericType(Type generic, Type type)
        {
            while (type != null && type != typeof(object))
            {
                ...
                if (generic.IsInterface)
                {
                    ...
                    //}
                    foreach (var found in type.GetInterfaces().Select(intfType => FindGenericType(generic, intfType)).Where(found => found != null))
                    {
                        return found;
                    }
                }
        ...
        {
            textPos = pos;
            _ch = textPos < textLen ? text[textPos] : '\0';
        }
        ...
        void NextToken()
        {
            ...
            _token.Id = t;
            _token.Text = text.Substring(tokenPos, textPos - tokenPos);
            _token.Pos = tokenPos;
        }
