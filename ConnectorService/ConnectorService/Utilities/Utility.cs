...
namespace ConnectorService.Utilities
{
    public class Utility
    {
        ...
        #region SerializationVO

        #region SerializeDadosTerminal

        //public string SerializeDadosTerminal(List<TbStock> valueStock, List<TbPClientes> valueClientes, List<TbPContaCorrente> valuecc, List<TbTaxasiva> valuetx)
        //public DadosTerminal SerializeDadosTerminal(List<St> valueStock, List<Cl> valueClientes, List<Cc> valuecc, List<Taxasiva> valuetx, List<Pn> valuePn)
        //{
        //    var resultado = new DadosTerminal
        //                        {
        //                            DadosClientes = valueClientes.Select(ConvertTbClVoCl).ToList(),
        //                            DadosStocks = valueStock.Select(ConvertTbStVoSt).ToList(),
        //                            DadosContasCorrentes = valuecc.Select(ConvertTbCcvoCc).ToList(),
        //                            DadosTaxasIva = valuetx.Select(ConvertTbTaxasIvaVoTaxasIva).ToList(),
        //                            DadosPn = valuePn.Select(ConvertTbPnVoPn).ToList(),
        //                            //DadosStobs = valueObs.Select(ConverTbStobsVoStobs).ToList()
        //                        };
        ...
        /// <returns></returns>
        public ConfiguracaoTerminal SerializeConfigTerminal(TbTerminais valueIft, TbOpcoesTerminal valueOpT)
        {
        ...
        #region SerializePNS
        /// <summary>
        /// metodo auxiliar para serializar os dados da pn
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        public DadosTerminal SerializePns(List<PnsFiltradas> value)
        {
            return new DadosTerminal
            {
                
                DadosPn = value.Select(ConvertTbPnVoPn).ToList()
            };
        }
        #endregion
        ...
        public ConfiguracaoTerminal ConvertTbConfigVoConfig(TbTerminais valueT, TbOpcoesTerminal valueop)
        {
            return new ConfiguracaoTerminal
            {
                IdConfiguracao = valueT.TerminalId.ToString(),
                NumPda = valueT.NumPda,
                NomeVendedor = valueop.NomeVendedor,
                NumVendedor = valueop.NumVendedor,
               
                FiltroCliente = valueop.PhcnFiltroCl,
                FiltroStock = valueop.PhcnFiltroAr,

                ArmazemEnvioEncomenda = valueop.Armenv,
                ArmazemRegistoEncomenda = valueop.Armreg,
                PhcNumDossier = valueop.PhcNumDi,
                PhcNomeDossier = valueop.PhcNomeDi,
                TipoRetencaoEncomendas = valueop.RetemEncomendas,
                NumeroEncomendasRetidas = valueop.NumEncomendasReter,
                NumeroInicioClientes= valueop.InitClientes,
                NumeroFimClientes= valueop.FimClientes,
                NumeroFimEncomendas = valueop.FimEncomendas,
                NumeroInicioEncomendas = valueop.InitEncomendas,

                EliminarEncomendas = Convert.ToInt32(valueop.EliminaEncomenda),
                AlterarPrecoVenda = Convert.ToInt32(valueop.AltPrecoVenda),
                IntroduzComponentes = Convert.ToInt32(valueop.IntCompSinc),
                MostrarDataLinhas = Convert.ToInt32(valueop.EncMostraData),
                CriaLinhaEncomenda = Convert.ToInt32(valueop.EncMostraData),
        ...
        #region ConvertPn
        /// <summary>
        /// metodo para converter entidade em vo(pn)
        /// </summary>
        /// <param name="value"></param>
        /// <returns></returns>
        private Vopn ConvertTbPnVoPn(PnsFiltradas value)
        {
            return new Vopn
                       {
                           PnDesign = value.design,
                           PnRef = value.PNReferencia,
                           PnNo = (int)value.no
        ...
        private bool ConvertDecimaltoBool(decimal ivaincl)
        {
        ...
        private decimal ConverteParamsStDec(string value)
        {
            decimal x = 0;
        ...
        private int ConverteParamsInt(string value)
        {
            var valor = 0;
